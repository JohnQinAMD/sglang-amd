...
class MLATokenToKVPoolTQ(MLATokenToKVPool):
    ...
    def _create_buffers(self):
        ...
        from sglang.srt.layers.quantization.turboquant_engine import (
            get_codebook, packed_bytes_per_dim,
        )

        tq_val = os.environ.get("SGLANG_KV_CACHE_TURBOQUANT", "4")
        self.tq_bit_width = int(tq_val) if tq_val not in ("1", "true", "True") else 4
        ...
        from sglang.srt.layers.quantization.turboquant_engine import generate_rotation_matrix
        self.tq_rotations = {}
        Pi_list = []
        for g in range(self.tq_n_groups):
            ...
            Pi_list.append(Pi)
        self.tq_Pi_all = torch.stack(Pi_list)  # (n_groups, g_dim, g_dim)
        ...
        try:
            ...
            so_candidates = [
                ...
                "/opt/turboquant/tq_kv_compress.so",
                "/shared_aig/john/semianalysis/aiter-amd/csrc/turboquant/tq_kv_compress.so",
            ]
            ...
            if so_path:
                ...
                self._tq_gpu_kernel = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(self._tq_gpu_kernel)
                logger.info(f"TurboQuant GPU kernel loaded from pre-compiled {so_path}")
            else:
                from torch.utils.cpp_extension import load
                src_candidates = [
                    "/opt/turboquant/turboquant_kv_compress.hip",
                    "/shared_aig/john/semianalysis/aiter-amd/csrc/turboquant/turboquant_kv_compress.hip",
                ]
                ...
                if src:
                    ...
                    if not os.path.exists(ck_inc):
                        ck_inc = "/opt/ck_include"
                    self._tq_gpu_kernel = load(
                    ...
                    logger.info("TurboQuant GPU kernel JIT-compiled")
        ...
        with self.memory_saver_adapter.region(GPU_MEMORY_TYPE_KV_CACHE):
            with (
                ...
                self.tq_compressed_bytes = packed_bytes + norms_bytes + rope_bytes
                self._tq_packed_end = packed_bytes
                self._tq_norms_end = packed_bytes + norms_bytes
        ...
        # O1: per-layer dirty tracking — skip full decompress when nothing changed
        self._deq_dirty = [True] * self.layer_num

        logger.info(
            f"TurboQuant MLA KV Pool: {self.tq_bit_width}-bit, "
    ...
    def _tq_compress(self, kv_data: torch.Tensor) -> torch.Tensor:
        """Compress KV data to packed format (2/3/4-bit)."""
        import math
        from sglang.srt.layers.quantization.turboquant_engine import (
        ...
        if self._tq_gpu_kernel is not None and self.tq_bit_width == 4:
            ...
            self._tq_gpu_kernel.turboquant_kv_compress_inplace(
                ...
                buf, self.tq_n_groups, self.tq_group_size,
            )
    ...
    def _tq_decompress(self, compressed: torch.Tensor) -> torch.Tensor:
        """Decompress packed data back to FP16/BF16 (2/3/4-bit)."""
        import math
        from sglang.srt.layers.quantization.turboquant_engine import (
        ...
        rope_raw = compressed[:, 0, self._tq_norms_end:]
    ...
    def get_key_buffer(self, layer_id: int):
        ...
        # O1: skip full decompress if this layer hasn't changed since last read
        if self._deq_dirty[layer_idx]:
            compressed = self.kv_buffer[layer_idx]
