...
def hip_make_swa_prefill_indices(
    ...
    Args:
        seq_lens_k:    int32 [batch_size] — total KV length per sequence.
        seq_lens_q:    int32 [batch_size] — query length per sequence (extend_seq_lens).
        swa_indices:   int32 [num_q_tokens, swa_window_size] (output, contiguous).
        cu_seqlens_q:  int32 [batch_size + 1] — cumulative qo lens (computed if None).
