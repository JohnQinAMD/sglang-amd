...
class FusedCompressMetadata(NamedTuple):
    write_loc: torch.Tensor  # shape [num_q_tokens]
    extra_data: Optional[torch.Tensor]  # shape [num_q_tokens] or None
    ...
    def copy_(self, other: FusedCompressMetadata) -> None:
        ...
        self.plan.copy_(other.plan)  # type: ignore
...
def create_paged_compressor_data(
    ...
    *,
    ...
    def get_raw_loc(positions: torch.Tensor) -> torch.Tensor:
        # NOTE: special case for overlap, we will handle it properly in kernel
        ...
        if envs.SGLANG_OPT_DEBUG_PAGED_COMPRESS.get():
            assert torch.all(state_loc % compress_ratio == 0)
        return (state_loc // compress_ratio).to(torch.int32)

    is_overlap = is_overlap_compress(compress_ratio)

    # NOTE(dark):
    # the spec of the following extra data is highly coupled with kernel implementation
    # DO NOT modify it unless you know what you are doing
    if is_prefill:
        assert extend_lens is not None
        if envs.SGLANG_OPT_TRITON_PREPARE_COMPRESS.get():
            ...
            load_positions = clip_down(prefix_lens - 1)
            write_loc = get_raw_loc(write_positions)
            load_loc = get_raw_loc(load_positions)
            if envs.SGLANG_OPT_DEBUG_PAGED_COMPRESS.get():
                assert torch.all(prefix_lens >= 0)
            if is_overlap:
                load_overlap_loc = get_raw_loc(load_positions - compress_ratio)
                write_overlap_loc = get_raw_loc(write_positions - compress_ratio)
                extra_data = torch.stack(
                    [
                        load_overlap_loc,
                        load_loc,
                        write_overlap_loc,
                        write_positions.to(torch.int32),
                    ],
                    dim=1,
                )
            else:
